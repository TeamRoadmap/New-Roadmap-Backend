...
funcs.validate = async (req, res, next) => {
  ...
  try {
    if (!validators[baseUrl]) {
      if (
        fs.existsSync(
          path.join(__dirname, "..", "utils", "validators", `${baseUrl}.js`)
        )
      ) {
    ...
    await Promise.all(
      validators[baseUrl][protocolName][endPointName].map((validation) =>
        validation.run(req)
      )
    );
    ...
    if (e && e.errors && e.errors.length) e.status = 400;

    return res
      .status((e && e.status) || 500)
      .send(
        e &&
          (e.hasOwnProperty("message") || e.hasOwnProperty("msg")) &&
